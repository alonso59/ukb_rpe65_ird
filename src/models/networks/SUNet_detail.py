...
if __name__ == '__main__':
    from model_utils import network_parameters

    height = 128
    width = 128
    x = torch.randn((1, 3, height, width)).cuda()
    model = SUNet(img_size=128, patch_size=2, in_chans=3, out_chans=1,
                  embed_dim=24, depths=[2, 2, 2, 2],
                  num_heads=[2, 2, 2, 2],
                  window_size=4, mlp_ratio=2., qkv_bias=True, qk_scale=4,
                  drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                  norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
