...
class SegmentationMetrics(object):
    ...
    def __init__(self, eps=1e-5, average=True, ignore_background=True, activation='softmax'):
        self.eps = eps
    ...
    def _calculate_multi_metrics(self, gt, pred, class_num):
        ...
        f1_score = (matrix[0] + self.eps) / (matrix[0] + (1 / 2) * (matrix[1] + matrix[2]) + self.eps)
        ...
        metrics = [pixel_acc, dice, precision, recall, f1_score]
        return metrics
    ...
    def __call__(self, y_true, y_pred):
        ...
        metrics = self._calculate_multi_metrics(gt_onehot, activated_pred, class_num)
        return metrics
